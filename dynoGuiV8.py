...
class MainWindowTest(QWidget):
	...
	def __init__(self):
		...
		self.chamber = None
		self.THERMAL_MAX_DELAY = 3000 # Maximum allowable delay to reach thermal equilibrium, will throw error if equalization takes longer
		self.THERMAL_POST_DELAY = 600 # Delay between tests at temp
	...
	def makeInputSide(self): 
		...
		timerLabel = QLabel('Delay Timer:')
		self.timerDisplay = QLCDNumber()
		pal = self.timerDisplay.palette()
		pal.setColor(pal.WindowText,QColor(255,0,0))
		self.timerDisplay.setPalette(pal)
		inputForm.addRow(timerLabel,self.timerDisplay)

		#self.leftSideLayout.addWidget(self.vIn)
		self.leftSideLayout.addLayout(inputForm)
		#self.leftSideLayout.addStretch()

		# self.leftSideLayout.addWidget(timerLabel)
	...
	def respondToStart(self):
		...
		for test in testList:
			...
			self.runSingleLoad(test.volt)

			myFile.close()

	def runSingleLoad(self,voltage):
		print('Single test')
		...
		scrubber_current = calcScrubCurrent(.062)
		self.Scrubber_Power.setCCVal(16,scrubber_current)
		self.liveDelay(2)
		pTime = time.time()
		i = 1
		while time.time() - pTime < 5:
			data = self.getData()
			self.displayData(data,voltage,i)
	...
	def runFullTest(self,voltage, FINAL_CURRENT):
		...
		while i < 15:
			...
			while dT < self.STABLE_DELAY:
				# update display
				if self.stopPressed:
					break
				data = self.getData()
				dT = time.time() - curTime
				self.timerDisplay.display(dT)
	...
	def getTorqueDist(self,maxVal):
		...
		desired = [0.125,.1,.1,.095,.07,.07,.06,.06,.06,.05,.05,.05,.04,.04,.03]
		actual = list()
		final = list()
		lastVal = 0
		for i in desired:
	...
	def getData(self):
		try:
			data = self.daq.get_data(timeout=0.5)
	...
	def calcScrubCurrent(desiredTorque):
		a = 59.6294
		b = -3.1274
		c = 0.0639-desiredTorque
		i1 = (-b+math.sqrt(b*b-4*a*c))/(2*a)
		i2 = (-b-math.sqrt(b*b-4*a*c))/(2*a)
		if i1 > 0:
			return i1
		else:
			return i2
...
class PopUpRun(QWidget):
	...
	def initUI(self):
		...
		self.scrubTorque.setTickInterval(.01)
		...
		self.stopPop = QPushButton('Stop Run',self)
		self.stopPop.clicked.connect(self.stopClicked)
		self.stopPop.setVisible(False)
