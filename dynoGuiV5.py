...
class MainWindowTest(QWidget):
	...
	def makeOutputSide(self):
		...
		self.vVal.resize(self.iVal.sizeHint())
		self.vVal.resize(self.pVal.sizeHint())
		self.vVal.resize(self.wVal.sizeHint())
		self.vVal.resize(self.tVal.sizeHint())
		self.vVal.resize(self.eVal.sizeHint())
	...
	def displayData(self,data,voltage):
	...
	def liveDelay(self,delayTime):
		...
		preTime = time.time()
		while time.time() - preTime < delayTime:
			QApplication.processEvents()
